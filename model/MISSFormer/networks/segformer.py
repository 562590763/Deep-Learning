...
__all__ = ['segformer_b0', 'segformer_b1', 'segformer_b2', 'segformer_b3', 'segformer_b4', 'segformer_b5']
...
class SegFormer(nn.Module):
    def __init__(self, dims, layers, embed_dim, image_size=224, num_classes=2):
        super().__init__()
        self.dims = dims
        self.embed_dim = embed_dim
        self.num_classes = num_classes
        self.backbone = MiT(image_size, dims, layers)
        self.segmentation_head = Decoder(dims, embed_dim, num_classes)
    ...
    def reset_classifier(self, num_classes):
        self.num_classes = num_classes
        self.segmentation_head = Decoder(self.dims, self.embed_dim, num_classes)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if x.size()[1] == 1:
            x = x.repeat(1, 3, 1, 1)
        encoder_outs = self.backbone(x)
        return self.segmentation_head(encoder_outs)


@register
def segformer_b0(args):
    dims = [32, 64, 160, 256]
    layers = [2, 2, 2, 2]
    embed_dim = 256
    model = SegFormer(dims=dims, layers=layers, embed_dim=embed_dim,
                      img_size=args.img_size, num_classes=args.num_classes)
    return model


@register
def segformer_b1(args):
    dims = [64, 128, 320, 512]
    layers = [2, 2, 2, 2]
    embed_dim = 256
    model = SegFormer(dims=dims, layers=layers, embed_dim=embed_dim,
