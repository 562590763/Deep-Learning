...
from model.MISSFormer.networks.segformer import TransformerBlock, \
    M_EfficientSelfAtten, MixFFN_skip, MixFFN, MiT

__all__ = ['missformer_base']
...
class MISSFormer(nn.Module):
    def __init__(self, heads, in_out_chan, dims, layers, reduction_ratios, d_base_feat_size,
                 num_classes=9, token_mlp_mode="mix_skip", encoder_pretrained=True):
        super().__init__()
        self.heads = heads
        ...
        self.segmentation_head = MyDecoderLayer((d_base_feat_size * 8, d_base_feat_size * 8), in_out_chan[0], heads[0],
    ...
    def reset_classifier(self, num_classes):
        ...
        self.segmentation_head = MyDecoderLayer((self.d_base_feat_size * 8, self.d_base_feat_size * 8),
                                                self.in_out_chan[0], self.heads[0],
                                                self.reduction_ratios[0], self.token_mlp_mode,
    ...
    def forward(self, x):
        ...
        # print("stage0-----")  
        tmp_0 = self.segmentation_head(tmp_1, bridge[0].permute(0, 2, 3, 1))
...
def missformer_base(args):
    ...
    d_base_feat_size = 7  # 16 for 512 inputsize, 7 for 224
    model = MISSFormer(heads=heads, in_out_chan=in_out_chan, dims=dims,
                       layers=layers, reduction_ratios=reduction_ratios,
