...
class OutputTransition(nn.Module):
    ...
    def forward(self, x):
        # convolve 32 down to num_classes channels
        out = self.relu1(self.bn1(self.conv1(x)))
        out = self.conv2(out)
...
class VNet(nn.Module):
    ...
    def __init__(self, num_classes, elu=True, nll=False):
        ...
        self.head = OutputTransition(32, num_classes, elu, nll)
    ...
    #     self.out_tr = OutputTransition(16, num_classes)
    def get_classifier(self):
        return self.head

    def reset_classifier(self, num_classes):
        self.num_classes = num_classes
        self.head = OutputTransition(32, num_classes, self.elu, self.nll)\
            if num_classes > 0 else nn.Identity()

    def forward(self, x):
        assert x.size()[1] == 1, "VNet needs data with one channel, got {}.".format(x.size()[1])
        assert len(x.size()) == 5, "VNet needs 3D data."
        out16 = self.in_tr(x)
