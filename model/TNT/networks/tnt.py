...
__all__ = ['tnt_s_patch16_224', 'tnt_b_patch16_224']
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, inner_dim=24, inner_stride=4):
        ...
        self.proj = nn.Conv2d(in_chans, inner_dim, kernel_size=(7, 7), padding=3, stride=(inner_stride, inner_stride))
...
class TNT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, outer_dim=384, inner_dim=24,
    ...
    def forward_features(self, x):
        B = x.shape[0]
        inner_tokens = self.patch_embed(x) + self.inner_pos  # B*N, 4*4, C_inner

        outer_tokens = self.proj_norm2(self.proj(self.proj_norm1(inner_tokens.reshape(B, self.num_patches, -1))))
        outer_tokens = torch.cat((self.cls_token.expand(B, -1, -1), outer_tokens), dim=1)  # B, N+1, C_outer
...
@register
def tnt_s_patch16_224(args, **kwargs):
    inner_stride = 4
    outer_dim = 384
    inner_dim = 24
    outer_num_heads = 6
    ...
    model = TNT(img_size=args.img_size, patch_size=args.patch_size, outer_dim=outer_dim, inner_dim=inner_dim, depth=12,
...
@register
def tnt_b_patch16_224(args, **kwargs):
    inner_stride = 4
    outer_dim = 640
    inner_dim = 40
    outer_num_heads = 10
