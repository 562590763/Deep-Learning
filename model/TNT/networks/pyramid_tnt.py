...
__all__ = ['ptnt_ti_patch16_192', 'ptnt_s_patch16_256', 'ptnt_m_patch16_256', 'ptnt_b_patch16_256']
...
class Block(nn.Module):
    ...
    def forward(self, x, outer_tokens, H_out, W_out, H_in, W_in, relative_pos):
        ...
        if self.has_inner:
            x = x + self.drop_path(self.inner_attn(self.inner_norm1(x.reshape(B, N, -1))
                                                   .reshape(B * N, H_in * W_in, -1), H_in, W_in))  # B*N, k*k, c
            x = x + self.drop_path(self.inner_mlp(self.inner_norm2(x.reshape(B, N, -1))
                                                  .reshape(B * N, H_in * W_in, -1)))  # B*N, k*k, c
            outer_tokens = outer_tokens + self.proj_norm2(self.proj(self.proj_norm1(x.reshape(B, N, -1))))  # B, N, C
        if self.se > 0:
            outer_tokens = outer_tokens + \
                           self.drop_path(self.outer_attn(self.outer_norm1(outer_tokens), H_out, W_out, relative_pos))
...
@register
def ptnt_ti_patch16_192(args, **kwargs):
    outer_dim = 80
    inner_dim = 5
    outer_head = 2
    inner_head = 1
    ...
    model = PyramidTNT(configs=configs, img_size=args.img_size, qkv_bias=False, **kwargs)
    model.default_cfg = default_cfgs['tnt_ti_patch16_192']
    return model


@register
def ptnt_s_patch16_256(args, **kwargs):
    outer_dim = 128
    inner_dim = 8
    outer_head = 4
    inner_head = 2
...
@register
def ptnt_m_patch16_256(args, **kwargs):
    outer_dim = 192
    inner_dim = 12
    outer_head = 4
    inner_head = 2
...
@register
def ptnt_b_patch16_256(args, **kwargs):
    outer_dim = 256
    inner_dim = 16
    outer_head = 4
    inner_head = 2
