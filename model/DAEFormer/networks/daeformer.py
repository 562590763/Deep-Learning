...
from model.MISSFormer.networks.segformer import TransformerBlock, \
    M_EfficientSelfAtten, MixFFN_skip, MixFFN, MLP_FFN, OverlapPatchEmbeddings

__all__ = ['daeformer_base']
...
class DAEFormer(nn.Module):
    def __init__(self, in_out_chan, dims, key_dim, value_dim, layers, d_base_feat_size,
                 num_classes=9, head_count=1, token_mlp_mode="mix_skip"):
        super().__init__()
        self.num_classes = num_classes
        self.in_out_chan = in_out_chan
        self.d_base_feat_size = d_base_feat_size
        self.head_count = head_count
        self.token_mlp_mode = token_mlp_mode

        # Encoder
        self.backbone = MiT(image_size=224, in_dim=dims, key_dim=key_dim, value_dim=value_dim,
        ...
        self.segmentation_head = MyDecoderLayer((d_base_feat_size * 8, d_base_feat_size * 8),
    ...
    def reset_classifier(self, num_classes):
        self.num_classes = num_classes
        self.segmentation_head = MyDecoderLayer((self.d_base_feat_size * 8, self.d_base_feat_size * 8),
                                                self.in_out_chan[0], self.head_count, self.token_mlp_mode,
                                                n_class=num_classes, is_last=True)
    ...
    def forward(self, x):
        ...
        tmp_0 = self.segmentation_head(tmp_1, output_enc[0].permute(0, 2, 3, 1))
...
def daeformer_base(args):
    ...
    model = DAEFormer(in_out_chan=in_out_chan, dims=dims, key_dim=key_dim, value_dim=value_dim, layers=layers,
                      d_base_feat_size=d_base_feat_size, num_classes=args.num_classes,
                      head_count=1, token_mlp_mode="mix_skip")
    return model
