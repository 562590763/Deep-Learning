...
from model.FTN.networks.transformer_block import Mlp
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(B, N, 3, self.num_heads, self.in_dim // self.num_heads).permute(2, 0, 3, 1,
                                                                                                  4)  # 3 B heads N C
...
class PSPModule(nn.Module):
    ...
    def forward(self, feats):
        n, hw, c = feats.size()
        feats = feats.contiguous().view(n, c, int(np.sqrt(hw)), int(np.sqrt(hw)))
